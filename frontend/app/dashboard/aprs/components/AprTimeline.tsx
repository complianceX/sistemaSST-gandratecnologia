...
const actionMap: Record<string, { label: string; icon: ReactNode }> = {
  APR_CRIADA: { label: 'APR criada', icon: <FilePlus2 className="h-4 w-4 text-amber-700" /> },
  APR_ATUALIZADA: {
    label: 'APR atualizada',
    icon: <RefreshCcw className="h-4 w-4 text-amber-600" />,
  },
  APR_PDF_ANEXADO: { label: 'PDF anexado', icon: <FileUp className="h-4 w-4 text-amber-700" /> },
  APR_FINALIZADA: {
  ...
  APR_CRIADA_POR_VERSAO: {
    ...
    icon: <FilePlus2 className="h-4 w-4 text-orange-600" />,
  },
};
