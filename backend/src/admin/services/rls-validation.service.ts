...
export class RLSValidationService {
    ...
    async testCrossTenantIsolation(
        ...
        try {
            ...
            await this.dataSource.query(
                `SET app.current_company = '${userCompanyId}'`,
            );
      ...
      `);
            ...
            const visibleCount = parseInt(result[0]?.count || 0);
            ...
            this.logger.error(
                `[CrossTenant] Test failed: ${error.message}`,
            );
     ...
     */
    async validateAdminCannotBypass(
        adminUserId: string,
    ): Promise<{
        status: 'secure' | 'vulnerable';
        message: string;
        ...
        this.logger.log(
            '[AdminBypass] Checking if admin can bypass RLS...',
        );
      ...
      `, ['public', CRITICAL_TABLES]);
            ...
            this.logger.error(`[AdminBypass] Check failed: ${error.message}`);
