...
export class EnterpriseValidationAndCompliance1709000000094 implements MigrationInterface {
  ...
  private formatCount(value: string | number | null | undefined): number {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const indexes = await this.queryRows<{ indexname: string }>(
      queryRunner,
      `
    ...
    const matviews = await this.queryRows<{ matviewname: string }>(
      queryRunner,
      `
    ...
    if (matviews.length > 0) {
      for (const view of matviews) {
        const rowCount = await this.queryRows<{ count: string }>(
          queryRunner,
          `SELECT COUNT(*) as count FROM ${view.matviewname}`,
        );
        console.log(
          `     └─ ${view.matviewname}: ${this.formatCount(rowCount[0]?.count)} rows`,
        );
      }
    }
    ...
    const triggers = await this.queryRows<{ trigger_count: string | number }>(
      queryRunner,
      `
    ...
    console.log(
      `   ✓ ${this.formatCount(triggers[0]?.trigger_count)} updated_at triggers active`,
    );
    ...
    const policies = await this.queryRows<{ policy_count: string | number }>(
      queryRunner,
      `
      SELECT COUNT(*) as policy_count
    ...
    if (this.formatCount(policies[0]?.policy_count) > 0) {
      const policyDetails = await this.queryRows<{
        table_name: string;
      ...
      for (const policy of policyDetails) {
        console.log(
          `     └─ ${policy.table_name}: ${this.formatCount(policy.retention_days)} days`,
        );
      }
    }
    ...
    const schemas = await this.queryRows<{ schema_name: string }>(
      queryRunner,
      `
    ...
    const ftsIndexes = await this.queryRows<{ indexname: string }>(
      queryRunner,
      `
      SELECT indexname FROM pg_indexes
