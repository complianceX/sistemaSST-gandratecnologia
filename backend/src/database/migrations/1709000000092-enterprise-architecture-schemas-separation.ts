...
export class EnterpriseArchitectureSchemasSeparation1709000000092
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      `);
        ...
        console.log(`   ✓ Created ${schemas.length} schemas: ${schemas.join(', ')}`);
        ...
        for (const [schema, tables] of Object.entries(this.schemaMapping)) {
            for (const table of tables) {
                ...
                try {
                    ...
                    console.warn(`   ⚠️  Could not move ${table}:`, error.message);
                }
            }
        }
        ...
        console.log('   GRANT SELECT ON ALL TABLES IN SCHEMA audit TO auditor_role;');
        console.log('');
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        const allTables = Object.values(this.schemaMapping).flat();
        for (const table of allTables) {
            await queryRunner.query(`DROP VIEW IF EXISTS "public"."${table}" CASCADE`);
        }
      ...
      `;
            ...
            try {
                const tables = await queryRunner.query(tableQuery);
